...
---
*Last updated: 2026-07-03 after Phase 4 (Brain Dump) completion — a user can offload a full head by text or voice, items auto-categorize into 5 buckets, and any one promotes straight into a Co-pilot session. Code-verified 12/12 (174 tests); two code-review criticals (hot-mic-on-unmount, dead promote button) caught and fixed. Device UAT (real Polish on-device STT, utterance segmentation, mic-denied fallback, hot-mic sanity) tracked in 04-HUMAN-UAT.md alongside the standing pre-Phase-9 iOS-device gate. NOTE: `expo-speech-recognition` added — `npx expo prebuild --clean` required after pulling.*
