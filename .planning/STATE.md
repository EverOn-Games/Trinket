...
- Phase 7 (Subscriptions): MONEY-01 sandbox purchase VERIFIED ON DEVICE 2026-07-05 — RevenueCat key + products + entitlement configured (entitlement identifier is `Trinket Pro`, supplied via `EXPO_PUBLIC_REVENUECAT_ENTITLEMENT` in `.env.local`); a real sandbox transaction granted the entitlement and the app now shows the subscriber state (entitlement-sync self-heal from quick task 260705-rbh confirmed working). STILL OPEN: restore-on-fresh-install + purchase-then-offline sequences (MONEY-03 device tests), sandbox-expiry downgrade observation, production-store product review, and MONEY-04 (Supabase account-at-purchase, deferred). ⚠ PRODUCTION LANDMINE: `.env.local` is dev-only/gitignored — `EXPO_PUBLIC_REVENUECAT_KEY`, `EXPO_PUBLIC_REVENUECAT_ENTITLEMENT`, and `EXPO_PUBLIC_POSTHOG_API_KEY` must also be set in EAS build env (eas.json profiles or EAS environment variables) or production builds silently fall back to reference/no-op mode.
