...
- [v0.2 synthesis ingested, 2026-07-05]: Founder uploaded trinket-dev-synthesis-v0.2.md (Phase 1 features). Bridge aligned to its §4 same-day. Soft landing is now SPECCED (§3: user-configured pre-transition informational notifications — supersedes the earlier 'needs founder definition' parking note). v0.2's own build order: light mode → Bridge → Soft landing → native surfaces (widgets/Live Activities/overlay, senior-hire slice). Companion form still under test — all surfaces stay companion-agnostic.
- [Bridge v0, 2026-07-05]: MECH-02 pulled forward pre-beta by founder decision — transitions are the brief's second hardest moment and nothing served it; v0 reuses promote/gate/ending primitives with zero new schema. MECH-01 (Soft landing) explicitly parked: never specced in the source doc; needs founder definition (decompression beat vs warm re-entry vs wind-down ritual) before build.
...
- Phase 7 (Subscriptions): MONEY-01 purchase loop verified on device 2026-07-05 **against RevenueCat's TEST STORE (simulated purchases — `test_…` key, NOT real Play Billing)**: purchase → `Trinket Pro` entitlement grant (id via `EXPO_PUBLIC_REVENUECAT_ENTITLEMENT` in `.env.local`) → paywall auto-dismiss → Settings tier flips live; expiry downgrade also observed. **Restore-on-fresh-install is structurally untestable on the Test Store** (no store account behind simulated purchases; a data-wipe mints a new anonymous RC user with nothing to restore — confirmed on device, wiring behaved correctly). STILL OPEN: real Play sandbox verification (RC Play Store app + `goog_` key, Play Console subscription products attached to the entitlement, internal-testing track + license tester), restore-on-fresh-install + purchase-then-offline on the real store, production-store product review, and MONEY-04 (Supabase account-at-purchase — FOUNDER-RATIFIED DEFERRAL 2026-07-05: not built for closed beta; anonymous purchases + store-account restore cover the beta reinstall case; revisit at production-prep primarily for opt-in backup and cross-platform entitlement). ⚠ PRODUCTION LANDMINES: (1) `.env.local` is dev-only/gitignored — `EXPO_PUBLIC_REVENUECAT_KEY`, `EXPO_PUBLIC_REVENUECAT_ENTITLEMENT`, and `EXPO_PUBLIC_POSTHOG_API_KEY` must also be set in EAS build env or production builds silently fall back to reference/no-op mode; (2) a `test_…` Test Store key in a submitted build = guaranteed App Review rejection AND an SDK crash in production — store builds must carry the `goog_`/`appl_` public keys.
...
| 260705-t50 | Paywall restore/purchase quiet outcome feedback (device UAT: Restore tap with nothing to restore looked like a dead button). Calm live-mode-only notices — restoreNone / purchaseIssue (EN+PL); cancelled stays noticeless; busy state double-tap-guards Choose/Restore. JS-only. 260 tests green. | 2026-07-05 | [paywall-restore-feedback](./quick/260705-t50-paywall-restore-purchase-quiet-outcome-f/) |
| 260705-u63 | Settings "Your plan" row now reactive (device UAT: granted purchase behind the pushed paywall left "Free" on screen until remount). `useTier()` zustand-subscribed hook in entitlements; getTier stays for event-time gate checks; RED-proofed regression test. JS-only. 261 tests green. | 2026-07-05 | [settings-tier-reactive](./quick/260705-u63-settings-tier-display-reactive-to-subscr/) |
| 260705-unb | Phase 9 pre-work (in-container halves of criteria 4+1): copy-tone gate (`lint:copy`, 346 strings clean, EN+PL shame/urgency/demand/medical-claim denylist with negation lookbehinds, RED-proofed) + network-isolation gate (`lint:network`, primitives ban + SDK-seam confinement, RED-proofed) + PostHog transport try/catch hardening + `docs/release-env.md` (EAS env commands, test_-key rejection warning). 262 tests green, four gates in verify. | 2026-07-05 | [phase-9-pre-work](./quick/260705-unb-phase-9-pre-work-copy-tone-gate-offline-/) |
| 260705-vl4 | Raccoon placeholder mascot: all 5 Lottie states regenerated as an actual raccoon character (generator committed at `scripts/generate-mascot-placeholders.mjs`, license-clean, 10-12KB/file, idle markers verbatim, seek-jolt rule held, no negative expressions). Visual QA via headless Chromium; one-shot tests now derive durations from assets. 262 tests green. Metro reload shows it — no prebuild. | 2026-07-05 | [raccoon-placeholder-mascot](./quick/260705-vl4-raccoon-placeholder-mascot-lottie-set/) |
| 260705-w3n | Manage-subscription link: Settings plus-tier row opens RevenueCat `managementURL` (store subscriptions page fallback) via new seam fn `getManagementUrl()` — billing lives with the store, with or without accounts. EN+PL, tests, 267 green. | 2026-07-05 | [manage-subscription-link](./quick/260705-w3n-manage-subscription-link-in-settings-via/) |
| 260705-war | Keep-awake during active Co-pilot session: presence holds the screen awake (expo-keep-awake, tag-scoped) while the mascot is awake; dozing (30 min) releases the hold as the battery valve; end/unmount releases. Already-bundled module — JS-only, no prebuild. 270 tests green. | 2026-07-05 | [keep-awake-session](./quick/260705-war-keep-screen-awake-during-active-co-pilot/) |
| 260705-wp9 | Bridge v0 (MECH-02, founder-ratified pre-beta pull-forward): post-ending "Anything next?" beat — ≤3 un-promoted dump items newest-first + equal-weight warm exit; item tap rides the gate-aware promote path; `bridge_next{startedNext}` allowlist event; zero-candidates → straight Home. Soft landing (MECH-01) stays parked pending founder definition. 275 tests green. | 2026-07-05 | [bridge-v0](./quick/260705-wp9-bridge-v0-post-ending-anything-next-offe/) |
| 260705-wxi | Bridge aligned to v0.2 spec §4 (founder-uploaded synthesis, persisted to .planning/source/): ritual added (breathing beat via presence state + localized 5-line self-compassion library in i18n + first-action framing), handoff triad (session/starter/close), unconditional post-ending offer, standalone /bridge route + Home link; bridge_next→{nextAction}. 277 tests green. | 2026-07-05 | [bridge-v0.2-spec](./quick/260705-wxi-bridge-to-v0-2-spec-ritual-handoff-triad/) |
| 260706-01x | Soft landing (MECH-01, v0.2 §3) — all 5 of 5 mechanics now exist: landings repo (no completion tracking by design), Starter-plumbing reuse, /soft-landing screen + Home link, informational EN/PL notification copy with plurals, opt-in transition touch, expiry-as-deletion, reminders-off sweep extended, landing_scheduled event, denylist probe extended. 288 tests green. | 2026-07-06 | [soft-landing](./quick/260706-01x-soft-landing-user-configured-pre-transit/) |
| 260706-089 | Light mode (POLI-01, v0.2 §5): authored lightTokens from the founder mockup CREAM/LIGHT reference (deeper text browns for contrast), new onAccent token fixing 20 latent cream-on-terracotta call sites (dark pixel-identical), themeMode System/Light/Dark switch in Settings (default system, dark fallback), pure resolveThemeTokens + tests, app.json userInterfaceStyle→automatic (**prebuild --clean required**), mascot-on-cream visually verified. 294 tests green. | 2026-07-06 | [light-mode](./quick/260706-089-light-mode-authored-light-tokens-system/) |
| 260706-0lc | Native surfaces (founder-scoped to widgets + Live Activities): SDK 56→57 (294 green first run), deep-link contract + surface_entry attribution, iOS resting-presence widget + session Live Activity (self-ticking timer text, zero updates) via official expo-widgets, lifecycle seam (start/end/orphan-sweep, session never owned by the surface), Android widget via react-native-android-widget (OPEN_URI + headless task + index.js entry), preview PNG from raccoon frame. 300 tests green. **prebuild --clean + dev-client rebuild required; iOS build = EAS/Mac gate.** | 2026-07-06 | [native-surfaces](./quick/260706-0lc-native-surfaces-scoped-sdk-57-expo-widg/) |
| 260706-0yf | iOS device-build guide (docs/ios-device-build.md): Windows→EAS→iPhone path, widget/Live Activity test steps, and the env split that bites — dev-client JS reads .env.local via Metro; preview/production bundle on EAS and need eas env vars (profiles now pinned to environments in eas.json). NSSupportsLiveActivities added. | 2026-07-06 | [ios-build-guide](./quick/260706-0yf-ios-device-build-guide-eas-env-wiring/) |
| 260706-ldj | v0.2 gap closure: Soft landing now has all three §3 entry points (task row link with prefill+sourceTaskId, quiet active-session link, standalone) + dozing-z recolored deep amber for light-mode legibility. 303 tests green. | 2026-07-06 | [gap-closure](./quick/260706-ldj-v0-2-gap-closure-landing-entry-points/) |
