...
**Confidence:** HIGH for versions/compatibility (verified via npm registry + official changelogs), MEDIUM for on-device classification approach (newer library, less battle-tested), LOW for STT module choice (fragmented ecosystem, no single dominant maintained option)

## The one thing to internalize before anything else

**"Expo managed workflow" in 2026 does not mean Expo Go.** Every native dependency this product needs — MMKV, RevenueCat, on-device STT, on-device text embeddings, Lottie (on some paths) — requires compiled native code. None of them run in the Expo Go sandbox app. The correct reading of the brief's "managed workflow as long as feasible" is:
...
This is not a deviation from the brief — RevenueCat, MMKV, and platform STT were always going to force this. Flag it explicitly so nobody burns a day debugging "why doesn't this work in Expo Go."
...
npx create-expo-app@latest trinket --template blank-typescript
cd trinket
...
# Analytics
npx expo install posthog-react-native expo-application expo-device expo-file-system expo-localization react-native-safe-area-context react-native-svg
...
npx eas build --profile development --platform all   # or `expo run:ios` / `expo run:android` locally
```
...
**If the on-device classification spike shows keyword matching is good enough (likely, given only 5 categories):**
...
**If SDK 57 proves stable with all native modules within the first few weeks of building:**
...
**If light mode is eventually added (post-MVP per the brief):**
...
---
*Stack research for: Trinket ADHD companion app (React Native + Expo)*
*Researched: 2026-07-01*
